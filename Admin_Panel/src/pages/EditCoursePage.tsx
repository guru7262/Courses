...
export function EditCoursePage() {
  ...
  const handleSaveBasicInfo = async () => {
    if (!subject || !selectedCategory) return;

    try {
      setSaving(true);

      // If category changed, we need to delete from old and add to new
      if (selectedCategory !== categoryId) {
        // Delete from old category
        await fetch(`${API_BASE_URL}/categories/${categoryId}/subject/${subject.id}`, {
          method: 'DELETE'
        });

        // Add to new category
        const response = await fetch(`${API_BASE_URL}/categories/${selectedCategory}/subject`, {
          method: 'POST',
        ...
        navigate(`/edit-course/${selectedCategory}/${subject.id}`);
      } else {
    ...
    if (hasChanges) {
      if (window.confirm('You have unsaved changes. Do you want to save them before editing content?')) {
        handleSaveBasicInfo().then(() => {
          navigate(`/edit-content/${selectedCategory}/${subjectId}`);
        });
      } else {
        navigate(`/edit-content/${selectedCategory}/${subjectId}`);
      }
    ...
    <>
      ...
      <div className="container">
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <div className="content-form" style={{ maxWidth: '900px', width: '100%' }}>
            ...
            <div className="form-group">
              ...
              <small style={{ color: '#666', fontSize: '0.85rem' }}>
                You can select an existing category or create a new one by entering a new category ID
              </small>
            </div>
            ...
            <div className="form-actions">
              <button className="btn btn-danger btn-small" onClick={handleDeleteCourse}>
                Delete Course
              </button>
